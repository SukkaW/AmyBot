...
async def lotto_parse():
	...
	for file in files:
		...
		winners= [x.text.replace("Winner: ","") for x in prizePane[0].find_all("div") if "Winner:" in x.text]
		prizes= [x.text.split()[2:] for x in prizePane[0].find_all("div") if "Prize:" in x.text]
